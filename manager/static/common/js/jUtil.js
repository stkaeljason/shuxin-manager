...
(function ($) {
    $.fn.extend({
        ...
        checkboxTree: function (data) {
            var iterator = function (node, $parent) {
                for (var i = 0; i < node.length; i++) {
                    var $li = $("<li class='list-group-item' data-id='" + node[i].id + "' data-txt='" + node[i].text + "'>" + node[i].text +
                        "<a class='glyphicon glyphicon-chevron-right pull-right add-block' title='添加版块'></a>" +
                        "<a class='glyphicon glyphicon-chevron-left pull-right remove-block' title='移除版块'></a>" +
                        "</li>");
                    $parent.append($li);
        ...
        throwTree: function (data, hasIndexModule) {
            var phone_submenu = [],
                phone_list = [];
            var iterator = function (node, $parent) {
                for (var i = 0; i < node.length; i++) {
                    if (node[i].node_type == 'phonemodel') {
                        ...
                        var $li = $("<li class='list-group-item' data-id='" + node[i].value + "' data-txt='" + node[i].name + "' data-type='" + node[i].node_type + "'>" + node[i].name +
                            "<a class='glyphicon glyphicon-chevron-right pull-right add-block' title='添加版块'></a>" +
                            "<a class='glyphicon glyphicon-chevron-left pull-right remove-block' title='移除版块'></a>" +
                            "</li>");
                        $parent.append($li);
            ...
            if (hasIndexModule && hasIndexModule === 1) {
                var featruedLi = $('<li class="list-group-item" data-txt="首页" data-type="featrued">' +
                    '<span class="j-switch" style="cursor:pointer">' +
                    '<i class="glyphicon glyphicon-plus"></i> ' +
                    '</span>首页&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;客户端选择 : ' +
                    '<select class="phone-list input-sm"></select>' +
                    '<a class="glyphicon glyphicon-chevron-right pull-right add-block" title="添加版块"></a>' +
                    '<a class="glyphicon glyphicon-chevron-left pull-right remove-block" title="移除版块"></a>' +
                    '<ul class="has-children" style="display: block;"></ul>' +
                    '</li>');
